...
class Dictionary:
    ...
    スタティックメソッド：
    line_to_pattern(str) -- パターン辞書読み込み用のヘルパー
    pattern_to_line(pattern) -- パターンハッシュをパターン辞書形式に変換する

    load_random(file) -- fileからランダム辞書の読み込みを行う
    load_pattern(file) -- fileからのパターン辞書の読み込みを行う
    load_template(file) -- fileからのテンプレート辞書の読み込みを行う
    load_markov(file) -- fileからのマルコフ辞書の読み込みを行う
    ...
    DICT_DIR = os.path.join(str(os.getcwd()), 'dics')
    ...
    def load_pattern(lines):
        ...
        return [Dictionary.line_to_pattern(x) for x in lines]

    @staticmethod
    @load_dictionary('template')
    def load_template(lines):
        """テンプレート辞書を読み込み，パターンハッシュを返す"""
    ...
    def _save_pattern(self):
        """パターン辞書を保存する"""
        lines = [Dictionary.pattern_to_line(x) for x in self._pattern]
        return '\n'.join(lines)
    ...
    @staticmethod
    def dicfile(key):
        """辞書のファイルパスを'DICT_DIR/DICT[key]'の形式で返す"""
